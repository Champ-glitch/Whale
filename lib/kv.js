...
export async function resetAll() {
  const prefixes = ["invoice:", "nickname:", "stats:", "refund:", "pendingpay:", "failcount:", "pendingreset:", "deduction:", "split:", "transfer:", "category_spent:", "weekly_saved:", "directpay:", "clientfund:", "adminpay:", "dailysnapshot:", "autoapprove:", "today:", "unclassified:"];
  let deletedCount = 0;
...
export async function getRecentActivity(limit = 4) {
  ...
  const directPayments = await listDirectPayments(50);

  const income = invoices
    .filter((i) => i.status === "success")
    .map((i) => ({ type: "in", amount: i.amount, label: i.description, at: i.createdAt || 0 }));

  const directIncome = directPayments.map((d) => ({ type: "in", amount: d.amount, label: d.label, at: d.at }));

  const outgoing = deductions.map((d) => ({ type: "out", amount: d.amount, label: d.reason, at: d.at }));

  const combined = [...income, ...directIncome, ...outgoing].sort((a, b) => b.at - a.at);
...
// ---- Direct payment log (Request Payment / Telegram /pay successes) ----
export async function logDirectPayment(amount, label) {
  const key = `directpay:${Date.now()}`;
  await kvCommand([
    'SET',
    key,
    JSON.stringify({ amount: Number(amount), label: label || 'Payment received', at: Date.now() }),
    'EX',
    60 * 60 * 48,
  ]);
}

export async function listDirectPayments(limit = 20) {
  const keys = await kvCommand(['KEYS', 'directpay:*']);
  if (!keys || keys.length === 0) return [];
  const items = [];
  for (const key of keys) {
    const value = await kvCommand(['GET', key]);
...
// ---- Today's activity (resets automatically each day via TTL) ----
function getTodayKey() {
  return new Date().toISOString().split("T")[0];
}

export async function recordTodayStats(amount) {
  const totalKey = `today:${getTodayKey()}:total`;
  const countKey = `today:${getTodayKey()}:count`;
  await kvCommand(["INCRBYFLOAT", totalKey, amount]);
  await kvCommand(["INCR", countKey]);
  await kvCommand(["EXPIRE", totalKey, 60 * 60 * 48]);
  await kvCommand(["EXPIRE", countKey, 60 * 60 * 48]);
}

export async function getTodayStats() {
  const totalKey = `today:${getTodayKey()}:total`;
  const countKey = `today:${getTodayKey()}:count`;
  const total = await kvCommand(["GET", totalKey]);
  const count = await kvCommand(["GET", countKey]);
  return { total: Number(total || 0), count: Number(count || 0) };
}

// ---- Unclassified payments (general public link) - awaiting merchant review ----
export async function saveUnclassified(reference, data) {
  await kvCommand([
    "SET",
    `unclassified:${reference}`,
    JSON.stringify(data),
    "EX",
    60 * 60 * 24 * 30, // 30 days to classify
  ]);
}

export async function listUnclassified(limit = 30) {
  const keys = await kvCommand(["KEYS", "unclassified:*"]);
  if (!keys || keys.length === 0) return [];
  const items = [];
  for (const key of keys) {
    const value = await kvCommand(["GET", key]);
    if (value) {
      items.push({ reference: key.replace("unclassified:", ""), ...JSON.parse(value) });
    }
  }
...
export async function removeUnclassified(reference) {
  await kvCommand(["DEL", `unclassified:${reference}`]);
}

// ---- Push notification subscriptions ----
export async function savePushSubscription(id, subscription) {
  await kvCommand(["SET", `pushsub:${id}`, JSON.stringify(subscription)]);
}

export async function listPushSubscriptions() {
  const keys = await kvCommand(["KEYS", "pushsub:*"]);
  if (!keys || keys.length === 0) return [];
  const items = [];
  for (const key of keys) {
    const value = await kvCommand(["GET", key]);
    if (value) {
      items.push({ id: key.replace("pushsub:", ""), subscription: JSON.parse(value) });
    }
  }
  return items;
}

export async function removePushSubscription(id) {
  await kvCommand(["DEL", `pushsub:${id}`]);
}
